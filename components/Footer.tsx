      ...
      <div className="rounded-2xl bg-forge-black-darker px-6 py-14 sm:px-10">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-[1.2fr_1fr_1fr_1fr_1fr]">
          <div className="flex flex-col gap-4">
            <p className="font-display text-lg font-medium text-pure-white">
          ...
          {footer.columns.map((column) => (
            <div key={column.heading} className="flex flex-col gap-4">
              <p className="font-display text-base font-medium text-white/60">
                {column.heading}
              </p>
              <ul className="flex flex-col gap-3">
                {column.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="font-display text-base text-pure-white transition-colors hover:text-forge-orange"
                    >
                      {link.label}
                    </Link>
                  </li>
...
function NewsletterForm() {
  // Capture-only for v1 per spec §8 — provider TBD (Master Spec §9).
  // Swap the onSubmit handler once a provider is chosen; markup won't need to change.
  return (
